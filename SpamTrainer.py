...
def installStopWords():
    dPath = input("Enter StopWord Path\n")
    dic = readFile(dPath)
    for word in dic.split():
...
def calHam(inp):
    return (inp + 0.5)/(totalHam + (0.5 * len(dbase)))


'Calculates the probability of spam words'
def calSpam(inp):
    return (inp + 0.5)/(totalSpam+(0.5 * len(dbase)))
...
'checks for stop words from an dictionary'
...
#F:\WorkSpace\SpamFilter\test\Sword.txt
